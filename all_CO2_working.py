...
def fix(df):
    start = pd.Timestamp(long(round(df.index[0].value, -11)))
    stop = pd.Timestamp(long(round(df.index[-1].value, -11)))
...
## CO2 inside
for f in  glob.glob('C:\Users\XPS\Dropbox\Hong Kong Tunnel\Upwind\CO2Inside\*.txt'):
    print f
    data = fix_CO2(f)
...
CO2_UO_data = []
## CO2 outside
for f in  glob.glob('C:\Users\XPS\Dropbox\Hong Kong Tunnel\Upwind\CO2Outside\*.txt'):
    print f
    data = fix_CO2(f)
...
appended_ALL_data.to_csv('CO2_data_rounded.csv')
